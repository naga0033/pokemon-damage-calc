...
exports.findBestJaMatch = findBestJaMatch;
exports.getJaMatchCandidates = getJaMatchCandidates;
...
function normalizeJaText(input) {
    return katakanaToHiragana(toHalfWidth(input)
        .normalize("NFKC")
        .replace(/[‐‑‒–—―ｰ]/g, "ー")
        .replace(/[()（）［］\[\]【】「」『』]/g, "")
        .replace(/[@＠]/g, "")
...
function findBestJaMatch(input, candidates, options) {
    ...
    const contains = normalizedCandidates.find((entry) => entry.normalized.includes(normalizedInput) ||
...
function getJaMatchCandidates(input, candidates, options) {
    const normalizedInput = normalizeJaText(input);
    if (!normalizedInput)
        return [];
