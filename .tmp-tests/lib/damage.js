...
exports.getKoLabelWithLeftovers = getKoLabelWithLeftovers;
const type_chart_1 = require("./type-chart");
const items_1 = require("./items");
const move_metadata_1 = require("./move-metadata");
 ...
 */
function calcDamage(input) {
    const { attackerLevel, move, defenderHp, attackerTypes, defenderTypes, attackerTeraType, isTerastallized, defenderTeraType, defenderTerastallized, isCritical, isBurned, weather, terrain, attackerItem, defenderItem, attackerAbility, defenderAbility, attackerRank, defenderRank, isCharged, } = input;
    ...
    if (move.type === "normal" && move.category !== "status") {
        if (attackerAbility === "dragon-skin") {
            effectiveMoveType = "dragon";
            skinMod = 1.2;
        } // ドラゴンスキン
        if (attackerAbility === "aerilate") {
            effectiveMoveType = "flying";
            skinMod = 1.2;
        } // スカイスキン
    ...
    if (defenderAbility === "volt-absorb" && effectiveMoveType === "electric" && !ignoresDefenderAbility)
        typeEff = 0;
    if (defenderAbility === "water-absorb" && effectiveMoveType === "water" && !ignoresDefenderAbility)
        typeEff = 0;
    if (defenderAbility === "flash-fire" && effectiveMoveType === "fire" && !ignoresDefenderAbility)
        typeEff = 0;
    if (defenderAbility === "sap-sipper" && effectiveMoveType === "grass" && !ignoresDefenderAbility)
        typeEff = 0;
    if (defenderAbility === "storm-drain" && effectiveMoveType === "water" && !ignoresDefenderAbility)
        typeEff = 0;
    if (defenderAbility === "lightning-rod" && effectiveMoveType === "electric" && !ignoresDefenderAbility)
        typeEff = 0;
    ...
    if (defenderAbility === "earth-eater" && effectiveMoveType === "ground" && !ignoresDefenderAbility)
        typeEff = 0;
    if (defenderAbility === "bulletproof" && BULLET_MOVES.has(move.name) && !ignoresDefenderAbility)
        typeEff = 0;
    ...
    // 攻撃側持ち物によるステータス補正
    const atkItem = items_1.ITEM_MAP[attackerItem];
    ...
    // 防御側持ち物によるステータス補正
    const defItem = items_1.ITEM_MAP[defenderItem];
    ...
    if (attackerAbility === "guts" && isBurned && move.category === "physical")
        abilityAtkMod = 1.5;
    if (attackerAbility === "gorilla-tactics" && move.category === "physical")
        abilityAtkMod = 1.5;
    ...
    if (attackerAbility === "protosynthesis" && weather === "sun") {
        if (input.attackerAbilityBoostedStat === "attack" && move.category === "physical")
            abilityAtkMod *= 1.3;
        else if (input.attackerAbilityBoostedStat === "spAtk" && move.category === "special")
            abilityAtkMod *= 1.3;
    }
    ...
    if (attackerAbility === "iron-fist" && PUNCH_MOVES.has(move.name))
        abilityAtkMod *= 1.2;
    if (attackerAbility === "strong-jaw" && BITE_MOVES.has(move.name))
        abilityAtkMod *= 1.5;
    if (attackerAbility === "mega-launcher" && PULSE_MOVES.has(move.name))
        abilityAtkMod *= 1.5;
    if (attackerAbility === "sharpness" && SLICING_MOVES.has(move.name))
        abilityAtkMod *= 1.5;
    if (attackerAbility === "punk-rock" && move.category !== "status" && move_metadata_1.SOUND_MOVES.has(move.name))
        abilityAtkMod *= 1.3;
    ...
    if (weather === "sun") {
        ...
        if (effectiveMoveType === "water")
            weatherMod = move.name === "hydro-steam" ? 1.5 : 0.5;
    }
    ...
    let terrainMod = 1;
    if (terrain === "electric" && effectiveMoveType === "electric" && attackerIsGrounded)
        terrainMod = 1.3;
    if (terrain === "grassy" && effectiveMoveType === "grass" && attackerIsGrounded)
        terrainMod = 1.3;
    if (terrain === "grassy" && GRASSY_HALVED.includes(move.name) && defenderIsGrounded)
        terrainMod = 0.5;
    if (terrain === "misty" && effectiveMoveType === "dragon" && defenderIsGrounded)
        terrainMod = 0.5;
    if (terrain === "psychic" && effectiveMoveType === "psychic" && attackerIsGrounded)
        terrainMod = 1.3;
    if (terrainMod !== 1) {
    ...
    // 攻撃側持ち物のダメージ補正
    let itemDmgMod = 1;
    if (atkItem?.effect === "life-orb")
        itemDmgMod = 1.3;
    if (atkItem?.effect === "expert-belt" && typeEff > 1)
        itemDmgMod = 1.2;
    if (atkItem?.effect === "type-boost" && atkItem.boostType === effectiveMoveType)
        itemDmgMod = 1.2;
    if (itemDmgMod !== 1) {
    ...
    if (defenderAbility === "punk-rock" && move.category !== "status" && move_metadata_1.SOUND_MOVES.has(move.name) && !ignoresDefenderAbility) {
        defAbilityMod *= 0.5;
    }
    ...
    const calcFinal = (rand, crit, extraMod = 1) => {
    ...
    const firstCritRolls = [];
    const firstNormalRolls = [];
    const laterCritRolls = [];
    const laterNormalRolls = [];
    for (let rand = 85; rand <= 100; rand++) {
...
function isGrounded(types, ability, item, teraType, isTerastallized) {
    const effectiveTypes = isTerastallized && teraType && teraType !== "stellar"
        ? [teraType]
        : types;
    if (item === "iron-ball")
        return true;
    ...
    if (ability === "levitate")
        return false;
    if (effectiveTypes.includes("flying"))
        return false;
    return true;
}
function getKoLabel(rolls, hp) {
    for (let n = 1; n <= 6; n++) {
        const minTotal = rolls[0] * n;
        const maxTotal = rolls[15] * n;
        if (minTotal >= hp)
            return `確定${toKanji(n)}発`;
        if (maxTotal >= hp) {
            // 何ロールでKOできるか
...
function getKoLabelWithLeftovers(rolls, hp) {
    if (!rolls || rolls.length === 0 || hp <= 0)
        return null;
...
function getMultiHitKoLabel(totalRolls, hp) {
