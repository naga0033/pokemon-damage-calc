...
export async function POST(req: NextRequest) {
  try {
    const { imageBase64, mediaType, ocrText } = await req.json();
...
${ocrText ? `補助情報としてローカルOCRの生テキストも渡します。OCRには誤認識が含まれるので、画像を最優先にしつつ、欠けた文字の補完にだけ使ってください。
OCRテキスト:
${ocrText}
` : ""}
...
6. 育成ツール/ビルダーの入力画面（性格補正のプルダウン、努力値/実数値のカード、持ち物の選択欄がある画面）
...
- 行ごとに "H 179 +228 / A 115 0 / B 147 +16 / C 58 0 / D 139 +252 / S 117 +12" のように並ぶレイアウトでは、左の数値を stats、"+" 付き右列を evs として読む
- 行ごとに "H 175 +116 / A 72 0 / B 94 +68 / C 175 +156 / D 126 +4 / S 155 +164" のように並ぶレイアウトでは、右列に "+" がない 0 も EV 0 として扱う
- 青い点はその能力が性格下降補正(0.9倍)、赤い点は性格上昇補正(1.1倍)。点が見えれば nature を推定する
- 持ち物がポケモン画像に重なった小さなアイコンで表示されている場合も読む。例: 紫の球体アイコンは「どくどくだま」
- ブーツのような装備アイコンが見える場合も item を推定する。例: ブーツなら「あつぞこブーツ」
- 左側の大きな1行は特性、その下のアイコン付き複数行は技として読む。特性行を技として読まない
- 右側の H/A/B/C/D/S 行は能力値エリアなので、そこに含まれる文字列や数字を技名として出力しない
- 技名は画像上で明確に読めるものだけ返す。怪しい文字列は技にしない
- 育成ツール画面で「性格補正」のプルダウンに性格名が表示されていれば、それを nature に入れる
- 育成ツール画面で「努力値 / 実数値」の各カードがある場合、各カードの左列を evs、右列を stats として読む
- 「道具」が「もちものを選択（なし）」なら item は空文字か null にする
