...
export const TYPE_NAMES_JA: Record<PokemonType, string> = {
  normal:   "ノーマル",
  fire:     "ほのお",
  water:    "みず",
  electric: "でんき",
...
export const ALL_TERA_TYPES: PokemonType[] = [
  "stellar",
  "normal","fire","water","electric","grass","ice","fighting","poison",
  "ground","flying","psychic","bug","rock","ghost","dragon","dark","steel","fairy",
];
...
export function getSingleEffectiveness(atk: PokemonType, def: PokemonType): number {
  return CHART[atk]?.[def] ?? 1;
}

/** 攻撃タイプ vs 防御タイプ（複数）の合計相性倍率 */
export function getTypeEffectiveness(atk: PokemonType, defTypes: PokemonType[]): number {
